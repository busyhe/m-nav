    ...
    <>
      {Object.keys(filteredData).map((type: string) => (
        <div key={type} id={type} className='mb-8 scroll-mt-20'>
          <h2 className='text-xl font-semibold mb-4 capitalize group flex items-center gap-2 relative'>
            <a
              href={`#${type}`}
              className='opacity-0 group-hover:opacity-100 transition-opacity text-muted-foreground hover:text-foreground absolute left-[-1rem]'
            >
              #
            </a>
            {type}
